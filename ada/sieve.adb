...
--  The program works by successively sending each number in 3..n through a
--  list of tasks; each task in the list corresponds to a prime number. The
--  list starts empty. As a number goes through the list, each task checks to
...
procedure Sieve is
   ...
   --  An access type so that tasks can be allocated dynamically.
...
begin
   ...
   declare
      First_Filter : constant Filter_Link := New_Filter (2);
   begin
