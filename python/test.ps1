...
$Error.clear()
python "$PSScriptRoot\anagrams.py" rats | Compare-Object (Get-Content "$PSScriptRoot\..\test\rats_heap_expected") | Assert-MatchTests &&
...
python "$PSScriptRoot\permutations.py" I like carrots | Compare-Object (Get-Content "$PSScriptRoot\..\test\carrots_expected") | Assert-MatchTests &&
...
Get-Content "$PSScriptRoot\..\test\wnba_input" | python "$PSScriptRoot\top_ten_scorers.py" | Compare-Object (Get-Content "$PSScriptRoot\..\test\wnba_expected") | Assert-MatchTests &&
python "$PSScriptRoot\triple_comprehensions.py" &&
python "$PSScriptRoot\triple.py" | Compare-Object (Get-Content "$PSScriptRoot\..\test\triple_expected") | Assert-MatchTests &&
python "$PSScriptRoot\types_are_types.py" &&
python "$PSScriptRoot\varargs.py" &&
python "$PSScriptRoot\vectors.py" &&
Get-Content "$PSScriptRoot\..\test\wordcount_ascii_input" | python "$PSScriptRoot\wordcount.py" | Compare-Object (Get-Content "$PSScriptRoot\..\test\wordcount_ascii_expected") | Assert-MatchTests &&
ForEach-Object 'foo';
